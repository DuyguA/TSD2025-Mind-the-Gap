...
model = FlaxWhisperForConditionalGeneration.from_pretrained("../do_train_segmented/whisper-puncted-segmented/85")
processor = WhisperProcessor.from_pretrained("../do_train_segmented/whisper-puncted-segmented/85")

input_features = processor(audio_30_sec, sampling_rate=sr, return_tensors="np")
print(input_features)

prompt_text = "dfdjdfjd"
#prompt_ids = processor.get_prompt_ids(prompt_text, return_tensors="pt")
prompt_ids = np.array([51909, 50364, 50364, 50364, 50364 ,51908])
print(prompt_ids)
...
outputs = model.generate(
    ...
    #forced_decoder_ids=prompt_ids[None, :],
)
...
decoded_output = processor.tokenizer.decode(predicted_token_ids, skip_special_tokens=False)
