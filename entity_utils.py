...
def tag_spacy_entities(normalized_words, puncted_words):
  entities = []
  puncted_sent  = "".join(puncted_words)

  doc = nlp(puncted_sent)
  ents = doc.ents
...
def does_ents_intersect(ent1_js, ent2_js):
  s1, e1 = ent1["start"], ent1["end"]
  s2, e2 = ent2["start"], ent2["end"]
...
def find_and_merge_intersecting_entities(entities1, entities2):
    ...
    for e1, e2 in merge_candidates:
        merged_entities.append(merge_entity_pair(e1, e2, tokens))
...
def tag_all_entities(normalized_words, puncted_words):
   if not normalized_words or not puncted_words:
     return []
   custom_ents = tag_custom_entities(normalized_words, puncted_words)
   spacy_ents = tag_spacy_entities(normalized_words, puncted_words)
