...
def process_all_test_instances(test_file_names, model_type):
  success_file =  "reports/sizes/" + model_type + ".txt"
...
'''
for alpha in (0.01, ):
    for beta in (0.0005, 0.001, 0.002, 0.005, 0.01):
        if alpha == 0 and beta == 0:
            continue
        directory = "whisper-" + str(alpha) + "-" + str(beta)
        modelnames.append(directory)
'''

checks = []
for wsize in [128, 256, 512, 1024, 2048, 4096]:
        checkd =  "whisper-0.0005-0.0005-" + str(wsize)
        modelnames.append(checkd)




for modeln in modelnames:
  process_all_test_instances(all_test_files, modeln)
