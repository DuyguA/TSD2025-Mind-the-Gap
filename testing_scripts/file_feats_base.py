...
from label_metrics_base import calculate_label_stats, calculate_all_label_stats
...
def process_single_pair(reference, hypothesis):
  ...
  label_stats = calculate_label_stats(aligned_ref_tokens, aligned_hyp_tokens, ref_bio_tags)
  ...
  return normalized_results, punct_cap_results, label_stats


  
def process_single_test_instance(base_name, ref_text, hypo_text):
  wer_results, punct_cap_results, label_stats = process_single_pair(ref_text, hypo_text)
  #print(wer_results)
  #print(punct_cap_results)
  #print(ner_results)
  return wer_results, punct_cap_results, label_stats
...
def process_all_test_instances(test_file_names, model_type):
  ...
  with open(success_file, "w") as osfile:
    for base_name in test_file_names:
      ...
      wer_results, punct_cap_results, label_stats = process_single_test_instance(base_name, ref_text, hypo_text)
...
process_all_test_instances(all_test_files, "base")
